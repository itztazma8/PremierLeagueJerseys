...
<body>
    ...
    <main>
        ...
        <br>
        <div class="a1">
            <img src="CSS/Kits/LIV_H.jpg" class="im">
            <div id="a3">
                <h1 class="t1">23/24 Liverpool Home Kit</h1>
                <p class="det">White contrast collar and cuffs nod to the LFC home shirt worn by the legendary ’74 squad - the re-birth of Shankly’s second team who triumphantly won the FA Cup. While a 97 set between two eternal flames resides on the back of the collar remembers the 97 people who tragically lost their lives at the Hillsborough Disaster.
</p>
                <p class="det1">Size:</p>
        ...
        <br>
        <div class="a1">
            <img src="CSS/Kits/LIV_A.png" class="im">
            <div id="a3">
                <h1 class="t1">23/24 Liverpool Away Kit</h1>
                <p class="det">Taking inspiration from LFC's iconic 95/96 Away Shirt, an all time fan favourite, Nikes innovative design breathes new life into the latest version for the memories in store for the 23/24 season, while a subtle detail below the back collar pays tribute to the 97 fans who tragically lost their lives at the Hillsborough Disaster.
</p>
                <p class="det1">Size:</p>
                <form method="post" class="ff">
                    <input type="radio" id="s" name="shirt" value="S">
...
if (isset($_POST['shirt']) || isset($_POST['home'])) {
    ...
    if (isset($_POST['but1'])) {
        ...
        if ($n==0) {
            if ($l1=='S') {
                ...
                VALUES ('LIV_H', '$l2', 95, '$l2',0,0,0,0)";
                mysqli_query($conn, $cc);
            }
            else if ($l1=='M') {
                
                $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
                VALUES ('LIV_H', '$l2', 95, 0,'$l2',0,0,0)";
                mysqli_query($conn, $cc);
            }
            else if ($l1=='L') {
                
                $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
                VALUES ('LIV_H', '$l2', 95, 0,0,'$l2',0,0)";
                mysqli_query($conn, $cc);
            }
            else if ($l1=='XL') {
                
                $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
                VALUES ('LIV_H', '$l2', 95, 0,0,0,'$l2',0)";
                mysqli_query($conn, $cc);
            } 
            else {
                
                $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
                VALUES ('LIV_H', '$l2', 95, 0,0,0,0,'$l2')";
                mysqli_query($conn, $cc);
            }
        }
    }
    ...
    if ($n==0) {
        if ($l1=='S') {
            $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
            VALUES ('LIV_A', '$l2', 85, '$l2',0,0,0,0)";
            mysqli_query($conn, $cc);
        }
        else if ($l1=='M') {
            
            $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
            VALUES ('LIV_A', '$l2', 85, 0,'$l2',0,0,0)";
            mysqli_query($conn, $cc);
        }
        else if ($l1=='L') {
            
            $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
            VALUES ('LIV_A', '$l2', 85, 0,0,'$l2',0,0)";
            mysqli_query($conn, $cc);
        }
        else if ($l1=='XL') {
            
            $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
            VALUES ('LIV_A', '$l2', 85, 0,0,0,'$l2',0)";
            mysqli_query($conn, $cc);
        } 
        else {
            
            $cc="INSERT INTO kits (name, no, price, S, M, L, XL, XXL)
            VALUES ('LIV_A', '$l2', 85, 0,0,0,0,'$l2')";
            mysqli_query($conn, $cc);
        }
    }
